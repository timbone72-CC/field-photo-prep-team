  ...
  if v_row.field_status = 'IN_PROGRESS' then
    update public.work_orders
       set field_status = 'FIELD_COMPLETE',
           field_completed_at = coalesce(field_completed_at, now())
     where id = p_work_order_id
     returning * into v_row;
...
revoke all on function private.start_work(uuid) from public, anon;
revoke all on function private.complete_field_work(uuid) from public, anon;
...
create or replace function public.start_work(p_work_order_id uuid)
returns table (
  work_order_id uuid,
  field_status text,
  started_at timestamptz,
...
set search_path = ''
as $$
  select * from private.start_work(p_work_order_id);
$$;

create or replace function public.complete_field_work(p_work_order_id uuid)
returns table (
  work_order_id uuid,
  field_status text,
  started_at timestamptz,
...
set search_path = ''
as $$
  select * from private.complete_field_work(p_work_order_id);
$$;

revoke all on function public.start_work(uuid) from public, anon;
revoke all on function public.complete_field_work(uuid) from public, anon;
grant execute on function public.start_work(uuid) to authenticated, service_role;
grant execute on function public.complete_field_work(uuid) to authenticated, service_role;
