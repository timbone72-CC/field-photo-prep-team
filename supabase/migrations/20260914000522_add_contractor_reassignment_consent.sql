drop function if exists public.admin_update_work_order(uuid, text, text, text, text, date, uuid);
drop function if exists private.admin_update_work_order(uuid, text, text, text, text, date, uuid);

alter table public.work_orders
  add column pending_assignee_user_id uuid references auth.users(id) on delete set null,
  add column reassignment_requested_at timestamptz;

create index work_orders_pending_assignee_user_id_idx
  on public.work_orders(pending_assignee_user_id);
...
create or replace function public.respond_reassignment(
  p_work_order_id uuid,
  p_accept boolean
)
returns table (
...
set search_path = ''
as $$
  select * from private.respond_reassignment(p_work_order_id, p_accept);
$$;

revoke all on function private.respond_reassignment(uuid, boolean) from public, anon;
revoke all on function public.respond_reassignment(uuid, boolean) from public, anon;
grant execute on function private.respond_reassignment(uuid, boolean) to authenticated, service_role;
grant execute on function public.respond_reassignment(uuid, boolean) to authenticated, service_role;
